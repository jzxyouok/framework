 ...
 * @since 2.0
 */
class Transaction extends \Leaps\Base\Object
{
	/**
	...
	public function begin()
	{
		if (!$this->_active) {
			...
			Leaps::trace('Starting transaction', __METHOD__);
			$this->db->open();
	...
	public function commit()
	{
		if ($this->_active && $this->db && $this->db->isActive) {
		    Leaps::trace('Committing transaction', __METHOD__);
			$this->db->pdo->commit();
			$this->_active = false;
		} else {
	...
	{
		if ($this->_active && $this->db && $this->db->isActive) {
			Leaps::trace('Rolling back transaction', __METHOD__);
