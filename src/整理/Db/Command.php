 ...
 * @since 2.0
 */
class Command extends \Leaps\Base\Component
{
	/**
	...
	public function execute()
	{
		...
		$token = $rawSql;
		try {
			Leaps::beginProfile($token, __METHOD__);

			$this->prepare();
			$this->pdoStatement->execute();
			$n = $this->pdoStatement->rowCount();

			Leaps::endProfile($token, __METHOD__);
			return $n;
		} catch (\Exception $e) {
			Leaps::endProfile($token, __METHOD__);
	...
	private function queryInternal($method, $fetchMode = null)
	{
		...
		if ($db->enableQueryCache && $method !== '') {
			$cache = is_string($db->queryCache) ? Leaps::$app->getComponent($db->queryCache) : $db->queryCache;
		}
		...
		if (isset($cache) && $cache instanceof Cache) {
			...
			if (($result = $cache->get($cacheKey)) !== false) {
				Leaps::trace('Query result served from cache', __METHOD__);
				return $result;
			}
		}
		...
		try {
			...
			if (isset($cache, $cacheKey) && $cache instanceof Cache) {
				...
				Leaps::trace('Saved query result in cache', __METHOD__);
			}
