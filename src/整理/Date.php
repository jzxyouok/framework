...
class Date
{
	...
	{
		return date ( $format ? $format : 'Y-m-d H:i:s', static::getTimeStamp ( $dateTime ) );
	}
	...
	{
		return date ( $interval, static::getTimeStamp ( $dateTime ) );
	}
	...
	{
		$date = getdate ( static::getTimeStamp ( $dateTime ) );
		switch ($interval) {
			case "y" :
				$date ["year"] += $value;
				break;
		...
		return static::format ( $format, mktime ( $date ["hours"], $date ["minutes"], $date ["seconds"], $date ["mon"], $date ["mday"], $date ["year"] ) );
	}
	...
	public static function getDaysInMonth($month, $year)
	{
		...
		if (! ($daysInmonths = static::getRealDaysInMonthsOfYear ( $year ))) {
			return 0;
		}
	...
	public static function getChinaDate($time = null)
	{
		...
		return sprintf ( '%s年%s月%s日(%s) %s%s:%s', $y, $m, $d, static::getChinaWeek ( $w ), static::getPeriodOfTime ( $h ), $_h, $i );
	}
	...
	{
		$oldTimezone = static::getTimezone ();
		if ('UTC' !== strtoupper ( $oldTimezone )) {
			static::setTimezone ( 'UTC' );
		}
		$date = date ( 'D, d M y H:i:s e', static::getTimeStamp ( $dateTime ) );
		if ('UTC' !== strtoupper ( $oldTimezone )) {
			static::setTimezone ( $oldTimezone );
		}
		return $date;
	}
	...
	{
		$timelang = ['second' => '秒前','yesterday' => '昨天','hour' => '小时前','minute' => '分钟前','qiantian' => '前天' ];
		$timestamp = $timestamp ? $timestamp : time ();
		$compareTime = strtotime ( static::format ( 'Y-m-d', $timestamp ) );
		$currentTime = strtotime ( static::format ( 'Y-m-d', $time ) );
		$decrease = $timestamp - $time;
		$result = static::format ( $format, $time );
		if (0 >= $decrease) {
			return 1 == $type ? [ static::format ( 'Y-m-d', $time ),$result ] : [ static::format ( 'Y-m-d m-d H:i', $time ),$result ];
		}
		if ($currentTime == $compareTime) {
			if (1 == $type) {
				...
				return 3600 >= $decrease ? [ ceil ( $decrease / 60 ) . $timelang ['minute'],$result ] : [ ceil ( $decrease / 3600 ) . $timelang ['hour'],$result ];
			}
			return [ static::format ( 'H:i', $time ),$result ];
		} elseif ($currentTime == $compareTime - 86400) {
			return 1 == $type ? [ $timelang ['yesterday'] . " " . static::format ( 'H:i', $time ),$result ] : [ static::format ( 'm-d H:i', $time ),$result ];
		} elseif ($currentTime == $compareTime - 172800) {
			return 1 == $type ? [ $timelang ['qiantian'] . " " . static::format ( 'H:i', $time ),$result ] : [ static::format ( 'm-d H:i', $time ),$result ];
		} elseif (strtotime ( static::format ( 'Y', $time ) ) == strtotime ( static::format ( 'Y', $timestamp ) )) {
			return 1 == $type ? [ static::format ( 'm-d', $time ),$result ] : [ static::format ( 'm-d H:i', $time ),$result ];
		}
		return 1 == $type ? [ static::format ( 'Y-m-d', $time ),$result ] : [ static::format ( 'Y-m-d m-d H:i', $time ),$result ];
	}
}