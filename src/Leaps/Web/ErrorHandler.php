...
class ErrorHandler extends \Leaps\ErrorHandler
{
	...
	public function renderFile($_file_, $_params_)
	{
		...
		if ($this->exception instanceof ErrorException || ! Kernel::$app->has ( 'view' )) {
			ob_start ();
			ob_implicit_flush ( false );
			extract ( $_params_, EXTR_OVERWRITE );
			require (Kernel::getAlias ( $_file_ ));

			return ob_get_clean ();
		} else {
			return Kernel::$app->getView ()->renderFile ( $_file_, $_params_, $this );
		}
	}
	...
	public function renderRequest()
	{
		...
		foreach ( [
			...
			if (! empty ( $GLOBALS [$name] )) {
				$request .= '$' . $name . ' = ' . var_dump ( $GLOBALS [$name] ) . ";\n\n";
			}
		}
